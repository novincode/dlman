...
export default function App() {
  ...
  const addPattern = () => {
    if (newPattern.trim() && settings) {
      // Ensure it starts with a dot
      let pattern = newPattern.trim().toLowerCase();
      if (!pattern.startsWith('.') && !pattern.startsWith('*')) {
        pattern = '.' + pattern;
      }
      if (!settings.interceptPatterns.includes(pattern)) {
        setSettings({
          ...settings,
          interceptPatterns: [...settings.interceptPatterns, pattern],
        });
      }
      setNewPattern('');
    }
  ...
  const removePattern = (pattern: string) => {
    if (settings) {
      setSettings({
        ...settings,
        interceptPatterns: settings.interceptPatterns.filter((p) => p !== pattern),
      });
    }
  };
  ...
  const resetPatterns = () => {
    if (settings) {
      setSettings({
        ...settings,
        interceptPatterns: DEFAULT_EXTENSION_SETTINGS.interceptPatterns,
      });
    }
  };
          ...
          <section className="mb-8">
            ...
            <div className="bg-card border rounded-lg p-4">
              <p className="text-sm text-muted-foreground mb-4">
                ...
                enabled. Add extensions like <code className="px-1 bg-muted rounded">.pdf</code> or patterns like <code className="px-1 bg-muted rounded">*.zip</code>.
              </p>
              ...
              <div className="flex gap-2 mb-4">
                <input
                  ...
                  placeholder="Enter extension (e.g., .pdf, .zip)"
                  className="flex-1 px-3 py-2 text-sm bg-background border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/50"
                />
                <button
                  onClick={addPattern}
              ...
              <div className="flex flex-wrap gap-2">
                {settings.interceptPatterns.map((pattern) => (
                  <span
                    key={pattern}
                    className="group flex items-center gap-1 px-2 py-1 bg-muted text-sm rounded-md hover:bg-muted/80 transition-colors"
                  >
                    {pattern}
                    <button
                      onClick={() => removePattern(pattern)}
                      className="ml-1 p-0.5 rounded hover:bg-destructive/20 hover:text-destructive transition-colors opacity-50 group-hover:opacity-100"
              ...
              {settings.interceptPatterns.length === 0 && (
                <p className="text-sm text-muted-foreground italic mt-2">
                  No patterns defined - all downloads will be intercepted
                </p>
              )}
            </div>
          </section>
